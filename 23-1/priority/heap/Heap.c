...
int GetPriChildIdx(Heap * h, int idx) {
    ...
    if (GetLeftChildIdx(idx) > h->numOfData) {  // 자식 노드가 존재하지 않으면
        ...
        if (h->arr[GetLeftChildIdx(idx)].prirority > h->arr[GetRightChildIdx(idx)].prirority) {
...
void HInsert(Heap * h, Data d, Prirority p) {
    ...
    int idx = ++(h->numOfData);
    HeapElement newElem = {d, p};

    while (idx != 1) {
        if (p < (h->arr[GetParentIdx(idx)].prirority)) {    // 부모 노드보다 우선 순위가 높다면
...
Data HDelete(Heap * h) {
    ...
    */
    while (childIdx = GetPriChildIdx(h, parentIdx)) {
        ...
        if (lastElem.prirority <= h->arr[childIdx].prirority) {
            break;
        }
